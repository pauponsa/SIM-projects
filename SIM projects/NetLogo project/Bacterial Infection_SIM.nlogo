;Hem afegit 11 slides perque l'usuari pugui determinar la quantiat de menjar que necessita cada una de les bacteries,
;la quantitat de menjar inicial que tindran cada pacient per les bacteries
;i la quantitat de menjar que anira apareixent a cada hora

breed [ antibiotics antibiotic ]
breed [ bacterias bacteria ]
breed [ dividers divider ]
breed [ foods food ] ;el que sha dafegir;


bacterias-own [
  variation
  my-target-patch
  ;Variable que determina la quantitat de menjar que ha injerit la bacteria
  food-quantity
  ;Variable que determina la velocitat de la bacteria
  speed-bacteria
  ;Variable que determina el menjar que necessita la bacteria per reproduir-se
  food-needed
]
;Hem afegit el menjar per ser el nou metode de reproduccio  de les bacteries
foods-own[
  my-target-patch
  menjat?
]
...
to setup
  ...
  ;Aquests son els metodes de inicialitzacio del menjar on el numero de menjar el decreta
  ; el usuari en la barra food-A i food-B
    create-foods food-A [
    let this-food self
    set shape ("molecule")
    set my-target-patch one-of patches with [ region = "A" ]
    move-to my-target-patch
    ask my-target-patch [ set my-food this-food]
    set size 1
    set color pink
  ];inicialitzacio food random
    create-foods food-B [
    let this-food self
    set shape ("molecule")
    set my-target-patch one-of patches with [ region = "B"]
    move-to my-target-patch
    ask my-target-patch [ set my-food this-food]
    set size 1
    set color pink
...
to make-bacteria [ number-of-bacteria which-variation which-region ]
  create-bacterias number-of-bacteria [
    ...
    ;tots aquests condicionals marquen el numero de menjar  que necessita per reproduir-se cada bacteri
    ; segons el numero de pores, a mes a mes determinem la velocitat dels bacteris segons la variacio
    if (which-variation = 3)[ ;;;;;;;;velocitat variants virus
      set speed-bacteria 0.4
      if( which-region = "A")[
      set food-needed init#-3-food-needed-a
      ]
       if( which-region = "B")[
      set food-needed init#-3-food-needed-b
      ]
    ]
    if (which-variation = 4)[
      set speed-bacteria 0.6
      if( which-region = "A")[
      set food-needed init#-4-food-needed-a
      ]
       if( which-region = "B")[
      set food-needed init#-4-food-needed-b
      ]
    ]
    if (which-variation = 5)[
      set speed-bacteria 0.7
      if( which-region = "A")[
      set food-needed init#-5-food-needed-a
      ]
       if( which-region = "B")[
      set food-needed init#-5-food-needed-b
      ]
    ]
    if (which-variation = 6)[
      set speed-bacteria 0.8
      if( which-region = "A")[
      set food-needed init#-6-food-needed-a
      ]
       if( which-region = "B")[
      set food-needed init#-6-food-needed-b
      ]
...
to go
  ...
  ;aquesta funcio fa la reproduccio dels bacteris
  reproduce-food-way
  move-antibiotics
  dose-each-region
  update-bacteria-counts
  ...
  update-time
  ; aquest condicional fa que a cada hora aparegui el numero de menjar
  ; que ha determinat el usuari per les dues regions
  if ticks mod 60 = 0[
    create-foods food-appear-per-hour [
      let this-food self
      set shape ("molecule")
      set my-target-patch one-of patches with [ region = "A" ]
      move-to my-target-patch
...
;En aquesta funcio hem canviat el metode de moviment de les bacteries
; Ara mateix cada bacteria es mou en una direccio i si xoca amb alguna paret
; canvia de direccio.
; Tambe comprova si els bacteris han de menjar, si es el cas, s'elimina el menjar
; i la variable quantiat de menjar del bacteri incrementa
to move-bacteria
  ...
  ;let speed-scalar 0.50 ; scales the speed of motion of the bacteria
  ;if my-distance-to-my-patch-center > 0.2 [
    ;set heading towards my-target-patch
    forward 0.1 * speed-bacteria
  ;]
  ask foods[
    let this-food self
    if any? bacterias-here [
      ask one-of bacterias-here [
        if eat-this-food? [
          ask this-food [ die ]
          set food-quantity ( food-quantity + 1)
        ]
      ]
    ]
  ]
...
;Hem modificat aquesta funcio perque els bacteris a part de rebotar a les parets dels costats,
;tambe rebotin a la de dalt i avall
to check-bounce-off-walls ; turtle procedure
  if member? pxcor (list (min-pxcor + 1) -2 2 (max-pxcor - 1)) [
    if member? [ pxcor ] of patch-ahead 0.5 (list min-pxcor -1 1 max-pxcor) [
      ...
      set heading (- heading - 1) ; if so, reflect heading around x axis
   ]
  ]
  if member? pycor (list (min-pycor + 1) 8 -8 (max-pycor - 1)) [
    if member? [ pycor ] of patch-ahead 0.5 (list min-pycor 10 -10 max-pycor) [
     ; back antibiotic-flow-rate
      set heading (random 360) ; if so, reflect heading around y axis
   ]
  ]
end

;Aquesta funcio es la original dels xocs de la paret que s'utilitza per les dosis
;perque no rebotin quan caiguin
to check-bounce-off-walls-antib ; turtle procedure
  if member? pxcor (list (min-pxcor + 1) -2 2 (max-pxcor - 1)) [
    if member? [ pxcor ] of patch-ahead 0.5 (list min-pxcor -1 1 max-pxcor) [
      ;back antibiotic-flow-rate
      set heading (- heading - 1) ; if so, reflect heading around x axis
...
;Aquest es el nou metode de reproduccio
;Quan la variable de menjar injerit es superior al necessitat,
;es reproduira i apareixera un nou bacteri del mateix tipus.
;Finalment reestablim les variables
to reproduce-food-way[]
  ask bacterias[
    let this-region region
    let this-bacteria self

    let available-patches neighbors with [ this-region = region ]

      if food-needed <= food-quantity [
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;Totes les funcions de les dosis no les hem tocat
to auto-dose
...
;Hem afegit aquest metode que funciona igual que el kill-this-bacteria pero pel menjar
to-report eat-this-food? ;new
  ; variations go from 3 to 6, for the number of holes in the membrane,
  ; so the odds of getting into the bacteria are 30%, 40%, 50% and 60%
  report random 100 < (variation * 10)
end
;Funcio ja no utilitzada
...
730
671
Initial bacterial food needed to reproduce for patient A
11
0.0
1
...
1170
676
Initial bacterial food needed to reproduce for patient B
11
0.0
1
...
Hem tractat el primer punt del extending model. Hem fet que els bacteris es puguin reproduir menjant i no segons el temps i que aquests es puguin moure. El menjar va reapareixent a cada hora segons la quanitat que l'usuari decideix i finalment hem determinat les velocitats de cada tipus de bacteri segons el número de pores, és a dir, el que té menys pores va més lent. 
En el extending anomena que els bacteris amb més pores haurien d'absorbir més quantitat de menjar més ràpid que els que en tenen menys. El que hem fet nosaltres és que l'usuari pugui decidir quina sigui la quantitat de menjar que ha d'absorbir cada tipus de bacteri.
